...
export default function Work({ setActiveTab }) {
  ...
  return (
    <div className="w-full bg-[#252422] min-h-screen space-y-20 pb-20 text-left antialiased selection:bg-[#EB5E28]/20 relative">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-[14px] font-bold text-xs tracking-wide transition-all duration-200 ease-out cursor-pointer ${
                filter === cat
                  ? 'bg-[#252422] text-[#EB5E28] shadow-[0_4px_12px_rgba(0,0,0,0.2)] scale-100'
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                key={project.title}
                className="bg-[#403D39] rounded-2xl overflow-hidden border border-white/[0.03] shadow-md hover:shadow-[0_12px_30px_rgba(0,0,0,0.25)] hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between group relative"
              >
                <div>
                  {/* Image Container with Grayscale Filter & Hover State */}
